...
func Run(repoUrl string, branch string, paths []Paths) error {
	...
	for _, path := range paths {
		if err := extractTarGzFromMemory(data, path.Extract, path.Repo); err != nil {
			return fmt.Errorf("unable to extract tar gz from memory for subPath '%s'\n%v", path.Repo, err)
		}
	}
...
func extractTarGzFromMemory(data []byte, dest string, subPath string) error {
	...
	for {
		...
		// Ensure only files that are within the basePath and subPath are processed.
